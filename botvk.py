...
class User:
    ...
    def _change_kb(self, current_list=[], flag=True):
        ...
        if flag:
            ...
            if current_list:
                ...
                for i in current_list:
                    self.keyboard.add(
                        Text(i), color=KeyboardButtonColor.POSITIVE)

                self.keyboard.row()

                (self.keyboard.add(Text("СТОП"), color=KeyboardButtonColor.NEGATIVE)
            ...
            if current_list:
                ...
                for i in current_list:
                    self.keyboard.add(
                        Text(i), color=KeyboardButtonColor.POSITIVE).row()
...
conn = sqlite3.connect(dir / "rate.db")
cursor = conn.cursor()
...
async def newUser(peer_id, name, surname):
    if users.get(peer_id) is not None:
        users.pop(peer_id)
...
def getRate():
    ...
    for i in users.keys():
        cursor.execute(
            """SELECT balance FROM vk_bd WHERE peer_id = (?)""", (i,))
        current_list.append(
            [str(users.get(i).name) + ' ' + users.get(i).surname, cursor.fetchone()[0]])
...
def addBal(amount, peer_id):
    cursor.execute(
        """SELECT balance FROM vk_bd WHERE peer_id = (?)""", (peer_id,))
...
def get_ex26(href):
    ...
    with open(dir / "lib" / href) as file:
        cur_answer = []
...
def shuf(current_list):
    shuffle(current_list)
    return current_list


def get_word_par(current_list):
    a = randint(0, len(current_list) - 1)
    ans = current_list[a]
    ...
    return m, c, v, current_list


def get_word_26(current_list):
    a = randint(0, len(current_list) - 1)
    ans = current_list[a]
    b = randint(0, len(ans[1]) - 1)
    c, v = ans[1][b], ans[0]
    ...
    return c, v, current_list


def get_word(current_list):
    ans = current_list[0]
    del current_list[0]
    return ans, current_list
...
async def function(message: Message):
    try:
        ...
        for i in range(len(nes_list)):
            str_bals += str(i + 1) + '. ' + \
                nes_list[i][0] + f' ({nes_list[i][1]})\n'
        await message.answer(message=str_bals)

    except BaseException:
        await message.answer(message='Проблемы с рейтингом')
    ...
    try:
        ...
        if this_user.busy == False:
            ...
            if message.text.lower() == 'стоп':
                ...
                if did_t - cor_t == 1:
                    await message.answer(message='Спишем на брак бота')
                elif did_t - cor_t == 0:
                    await message.answer(message='Думаю, тут минимум 92 балла в формате ЕГЭ')
                elif did_t - cor_t < 3:
                    await message.answer(message='Not bad')
                else:
                    if this_user.name == 'Илья':
                        await message.answer(message='Даже не верится, что ты Илья 0_0')
                    else:
                        await message.answer(message='Ты не Илья, тебе можно')
                ...
                ) == check_word[cord_start:cord_end].lower():
                    cor_t += 1
    ...
    try:
        ...
        if this_user.busy == False:
            ...
            if message.text.lower() == 'стоп':
                ...
                check_word = this_user.check_word
                par_kb, get_word_ans, par_word, new_words = get_word_par(
                    new_words)
                this_user.words = new_words
                this_user._change_kb(par_kb, False)
...
@bot.on.private_message(func=lambda message: message.text.lower()
                        != 'меню' and message.text.lower() != '[club187730402|@botaiege]')
async def function(message: Message):
    users_info = await bot.api.users.get(message.from_id)
